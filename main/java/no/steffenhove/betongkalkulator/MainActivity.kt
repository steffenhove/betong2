...
class MainActivity : AppCompatActivity() {
    ...
    fun MainActivityContent(context: Context, unitSystem: String) {
        var diameterState by remember { mutableStateOf("") }
        ...
        Column(Modifier.padding(16.dp)) {
            when {
                coreVisible -> KjerneInput(onCalculateClick = { diameter, height, diameterUnit, heightUnit ->
                    val diameterDouble = diameter.toDoubleOrNull() ?: 0.0
                    ...
                    resultText = calculateAndSave(context, "Kjerne", coreDimensions = coreDimensions)
                })
                squareVisible -> FirkantInput(onCalculateClick = { length, width, thickness, lengthUnit, widthUnit, thicknessUnit ->
                    ...
                    resultText = calculateAndSave(context, "Firkant", squareDimensions = squareDimensions)
                })
                triangleVisible -> TrekantInput(onCalculateClick = { a, b, c, thickness, sideAUnit, sideBUnit, sideCUnit, thicknessTriangleUnit ->
                    ...
                    resultText = calculateAndSave(context, "Trekant", triangleDimensions = triangleDimensions)
                })
            }
    ...
    private fun calculateCuboidVolume(length: Double, width: Double, thickness: Double): Double {
        ...
        val density = when (prefs.getString("density_preference", "Betong")) {
            ...
            "Egendefinert" -> {
                ...
                customDensityString?.toDoubleOrNull()?.takeIf { it > 0 } ?: run {
                    Toast.makeText(this, "Ugyldig tetthet for Egendefinert. Bruker standardverdi (2400 kg/m³)", Toast.LENGTH_SHORT).show()
                    DENSITY_CONCRETE
                }
            }
